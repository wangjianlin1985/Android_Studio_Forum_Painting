...
public class PaintUserAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.paint_user_add); 
		...
		iv_paintPhoto.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(PaintUserAddActivity.this,photoListActivity.class);
		...
		btn_paintFile.setOnClickListener(new OnClickListener() { 
			...
			public void onClick(View arg0) {
				Intent intent = new Intent();
				intent.setClass(PaintUserAddActivity.this,fileListActivity.class);
				startActivityForResult(intent,REQ_CODE_SELECT_FILE_paintFile);
			}
		});
		
		Declare declare = (Declare) PaintUserAddActivity.this.getApplication();
		paint.setUserObj(declare.getUserName()); 
			...
			public void onClick(View v) {
				try {
					...
					if(ET_paintName.getText().toString().equals("")) {
						Toast.makeText(PaintUserAddActivity.this, "作品名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(paint.getPaintPhoto() != null) {
						...
						PaintUserAddActivity.this.setTitle("正在上传图片，稍等...");
						String paintPhoto = HttpUtil.uploadFile(paint.getPaintPhoto());
						PaintUserAddActivity.this.setTitle("图片上传完毕！");
					...
					if(ET_paintDesc.getText().toString().equals("")) {
						Toast.makeText(PaintUserAddActivity.this, "作品描述输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(paint.getPaintFile() != null) {
						...
						PaintUserAddActivity.this.setTitle("正在上传文件，稍等...");
						String paintFile = HttpUtil.uploadFile(paint.getPaintFile());
						PaintUserAddActivity.this.setTitle("文件上传完毕！");
					...
					paint.setAddTime("--");
					/*调用业务逻辑层上传绘画信息*/
					PaintUserAddActivity.this.setTitle("正在上传绘画信息，稍等...");
